...
def train():
    ...
    history = trainer.train(dataset_train, dataset_valid, epoch=52, train_steps=int(np.ceil(5200 / 100)),
                  valid_steps=int(np.ceil(260 / 100)), dis_show_bar=True)
...
def test():
    ...
    result, label = trainer.test(dataset_test, test_steps=int(np.ceil(1300 / 64)), dis_show_bar=True)
